...
function solve(cadenaP) {
    ...
    let operations = 0;
    const result = NearPalindrome(cadenaP);
...
function NearPalindrome(cadenaP) {

    const contarChar = {};
    for (const char of cadenaP) {
        contarChar[char] = (contarChar[char] || 0) + 1;
        console.log(contarChar)
    }
    //  near-palindrome tiene más de un carácter con frecuencia impar
    let impar = 0;
    for (const char in contarChar) {
        if (contarChar[char] % 2 !== 0) {
            impar++;
        }
    }
    return impar <= 1;
